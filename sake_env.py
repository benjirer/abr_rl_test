...
from sake_model import SakeModel
...
class SakeEnv(BaseEnv):
    ...
    def process_env_info(self, env_info: dict) -> dict:
        ...
        if (
        ...
        ):  # avoid doing extra work on zeros for very first action
            ...
            env_info["past_chunk"]["delay"] = max(
                env_info["past_chunk"]["delay"], 1e-6
            )  # avoid division by 0
            throughput = (
                env_info["past_chunk"]["size"] / env_info["past_chunk"]["delay"]
            )
        ...
        video_sizes = normalize(np.array(env_info["sizes"]), max=self.max_video_size)
        ssim_dbs = [
        ...
        obs = {
            key: th.as_tensor(val, dtype=th.float32).unsqueeze_(0)
