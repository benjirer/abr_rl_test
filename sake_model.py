...
class SakeModel(nn.Module):
    ...
    def __init__(
        self, look_ahead_horizon: int = 5, n_actions: int = 10, feature_dim: int = 897
    ):
        super(SakeModel, self).__init__()
    ...
    def forward(self, observation: dict) -> int:
        ...
        action = th.argmin(dist.probs, dim=1)
        return action.item()
